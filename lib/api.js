...
export async function getFeaturedStories(ids) {
  const authorInfoIds = {};
  const featuredStories = {};
  const featuredStoriesData = await client.getByIDs(ids);
  featuredStoriesData.results.map((item) => {
    authorInfoIds[item.data.author_info.id] = item.id;
    featuredStories[item.id] = {
      id: item.id,
      uid: item.uid,
      preview_title_regular: item.data.story_preview_title_regular,
      preview_title_italic: item.data.story_preview_title_italic,
      preview_text: item.data.story_preview_text,
      preview_image: item.data.preview_image,
    };
  });

  const signatures = await getAuthorSignatureFromIds(
    Object.keys(authorInfoIds),
  );

  for (const authorInfoId in signatures) {
    const storyId = authorInfoIds[authorInfoId];
    featuredStories[storyId]['signature'] = signatures[authorInfoId];
  }

  return Object.values(featuredStories);
}
