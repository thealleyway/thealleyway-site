...
export async function getValues(ids) {
  const values = await client.getByIDs(ids);
  const results = values.results.map((item) => {
    return {
      id: item.id,
      title: item.data.value_title,
      description: item.data.value_description,
    };
  });
  return results
