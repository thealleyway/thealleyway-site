...
export async function getFeaturedStories(ids) {
  const featuredStories = await client.getByIDs(ids);
  const results = featuredStories.results.map((item) => {
    return {
      id: item.id,
      storyTitle: item.data.story_title,
      storyDate: item.data.story_date,
      authorInfo: item.data.author_info,
      previewImage: item.data.preview_image,
    };
  });
  return results;
}
